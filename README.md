...
## GitHub Pages plot dashboard (auto-updating)

This repository includes a complete GitHub Pages dashboard that automatically discovers
every HTML file under `Paper_results/` (including nested folders), builds navigation/cards,
and embeds each plot in an iframe.

### How it works

- Frontend template files live in `pages/`.
- Build script `scripts/build_pages.py`:
	- recursively scans `Paper_results/` for `*.html`
	- copies `Paper_results/` into a deployable `site/` folder
	- generates `site/plots-manifest.json`
- GitHub Actions workflow `.github/workflows/pages.yml` runs on push and deploys `site/` to Pages.
...
After that, any push that changes `Paper_results/`, `pages/`, or the build workflow/script
will automatically redeploy the site. No manual plot registration is needed.
...
Then open `site/index.html` in a browser (or serve `site/` with a simple static server).

## Citation for papers

You can cite this work in two ways:

- Website citation directly from the dashboard:
	- open the deployed GitHub Pages site
	- copy APA or BibTeX from the **Cite This Website** panel
- Repository citation metadata via GitHub:
	- use the repository **Cite this repository** button
	- metadata comes from `CITATION.cff`

The dashboard citation includes the current plot count and access date for reproducible reporting.
